...
DNS_SYNC_SCRIPT = "/usr/local/bin/dns-nft-sync.sh"
DNS_SYNC_SERVICE_FILE = "/etc/systemd/system/dns-nft-sync.service"
DNS_SYNC_TIMER_FILE = "/etc/systemd/system/dns-nft-sync.timer"
DNS_SYNC_TIMER_NAME = "dns-nft-sync.timer"
...
DNS_SYNC_SCRIPT_CONTENT = r"""#!/usr/bin/env bash
# DNS -> nftables set sync + DNAT rule refresh (auto-generated by nft-forward-panel)
# usage: dns-nft-sync.sh
...
if [[ "$changed" -eq 1 ]]; then
    ...
    {
        echo "#!/usr/sbin/nft -f"
        echo "# DNS dynamic forward - auto-generated $(date '+%Y-%m-%d %H:%M:%S')"
...
def ensure_dns_sync_timer():
    try:
        with open(DNS_SYNC_SCRIPT, "w", encoding="utf-8") as f:
            f.write(DNS_SYNC_SCRIPT_CONTENT)
        os.chmod(DNS_SYNC_SCRIPT, 0o755)
        need_reload = False
        if not os.path.exists(DNS_SYNC_SERVICE_FILE):
            with open(DNS_SYNC_SERVICE_FILE, "w", encoding="utf-8") as f:
                f.write(DNS_SYNC_SERVICE_CONTENT)
            need_reload = True
        if not os.path.exists(DNS_SYNC_TIMER_FILE):
            with open(DNS_SYNC_TIMER_FILE, "w", encoding="utf-8") as f:
                f.write(DNS_SYNC_TIMER_CONTENT)
            need_reload = True
        if need_reload:
            sh(["systemctl", "daemon-reload"])
        sh(["systemctl", "enable", "--now", DNS_SYNC_TIMER_NAME])
        sh([DNS_SYNC_SCRIPT])
        log("dns sync timer ensured")
    except OSError as e:
        log("failed to ensure dns sync timer: %s" % e)

def disable_dns_sync_timer():
    try:
        sh(["systemctl", "disable", "--now", DNS_SYNC_TIMER_NAME])
    except OSError:
        pass
