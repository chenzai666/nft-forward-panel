...
*{box-sizing:border-box}body{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--page);color:var(--ink);font-size:14px}
...
.wrap{max-width:1280px;margin:0 auto;padding:24px}.toolbar{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:18px}.title h1{font-size:24px;margin:0 0 4px}.title p{margin:0;color:var(--muted)}.actions{display:flex;gap:10px;flex-wrap:wrap}
...
.form{padding:16px;display:grid;gap:12px}.field{display:grid;gap:6px}.field label{font-size:12px;color:#526071;font-weight:750}input{height:38px;border:1px solid #cbd5e1;border-radius:7px;padding:0 10px;font-size:14px;background:#fff;color:#111827}input:focus{outline:2px solid #bfdbfe;border-color:#60a5fa}.twocol{display:grid;grid-template-columns:1fr 1fr;gap:10px}.side-note{padding:12px 14px;margin:0 16px 16px;border-radius:8px;background:#f8fafc;border:1px solid var(--line);color:#64748b;font-size:13px}
...
@media(max-width:980px){.status-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.layout{grid-template-columns:1fr}.toolbar{align-items:flex-start;flex-direction:column}.actions{width:100%}.actions button{flex:1}.topbar{padding:0 16px}}
@media(max-width:680px){.wrap{padding:16px}.status-grid{grid-template-columns:1fr}.twocol{grid-template-columns:1fr}.topmeta{display:none}th{display:none}tr{display:block;border-bottom:1px solid #edf0f5;padding:9px 0}td{display:flex;justify-content:space-between;gap:14px;border:0;padding:7px 14px}td:before{content:attr(data-label);color:#64748b;font-size:12px;font-weight:700}.namecell{font-weight:800}}
</style></head><body><div class="shell"><header class="topbar"><div class="brand"><div class="mark">N</div><span>nftables 转发面板</span></div><div class="topmeta"><span id="last">等待同步</span></div></header>
<main class="wrap"><div class="toolbar"><div class="title"><h1>转发规则控制台</h1><p>当前规则、服务状态与重载操作集中在这里。</p></div><div class="actions"><button class="ghost" onclick="load()">刷新</button><button class="secondary" onclick="reloadRules()">重载 nft</button></div></div>
<div class="status-grid"><div class="metric"><div class="label">nftables</div><div class="value" id="nftState">-</div><div class="hint">系统服务状态</div></div><div class="metric"><div class="label">Web 面板</div><div class="value" id="panelState">-</div><div class="hint">systemd 运行状态</div></div><div class="metric"><div class="label">普通转发</div><div class="value" id="portCount">0</div><div class="hint">DNAT 规则数量</div></div><div class="metric"><div class="label">DNS 转发</div><div class="value" id="dnsCount">0</div><div class="hint">域名规则数量</div></div></div>
<div id="msg" class="msg"></div><div class="layout"><section class="panel"><div class="panel-head"><h2>规则列表</h2><span class="badge blue" id="totalBadge">0 条</span></div><div class="tabs"><button class="tab active" onclick="switchTab('port')">普通转发</button><button class="tab" onclick="switchTab('dns')">DNS 转发</button></div><div class="panel-body"><div id="portPane" class="tabpane active"><div id="rules"></div></div><div id="dnsPane" class="tabpane"><div id="dns"></div></div></div></section>
<aside class="panel"><div class="panel-head"><h2>新增规则</h2><span class="badge green">自动重载</span></div><div class="tabs"><button class="tab active" onclick="switchForm('port')">普通</button><button class="tab" onclick="switchForm('dns')">DNS</button></div><div id="portForm" class="form"><div class="field"><label>名称</label><input id="name" placeholder="例如 web-api"></div><div class="twocol"><div class="field"><label>本机端口</label><input id="lport" placeholder="10000"></div><div class="field"><label>目标端口</label><input id="dport" placeholder="443"></div></div><div class="field"><label>目标 IPv4</label><input id="dip" placeholder="1.2.3.4"></div><button onclick="addRule()">添加普通转发</button></div><div id="dnsForm" class="form" style="display:none"><div class="field"><label>域名</label><input id="domain" placeholder="example.com"></div><div class="twocol"><div class="field"><label>本机端口</label><input id="dlport" placeholder="10001"></div><div class="field"><label>目标端口</label><input id="ddport" placeholder="443"></div></div><div class="field"><label>nft set</label><input id="setname" placeholder="可留空"></div><button onclick="addDns()">添加 DNS 转发</button></div><p class="side-note">面板会写入现有 nftables 配置并触发重载。</p></aside></div></main></div>
<script>
