...
<title>nftables Forward Panel</title>
<style>
:root{--ink:#111827;--muted:#64748b;--line:#d9dee8;--panel:#fff;--page:#f3f5f8;--nav:#111827;--blue:#2563eb;--green:#059669;--amber:#b45309;--red:#dc2626;--soft-blue:#eaf1ff;--soft-green:#e8f7ef;--soft-amber:#fff4df}
*{box-sizing:border-box}html,body{width:100%;overflow-x:hidden}body{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--page);color:var(--ink);font-size:14px}
.shell{min-height:100vh}.topbar{height:64px;background:var(--nav);color:#fff;display:flex;align-items:center;justify-content:space-between;padding:0 28px}.brand{display:flex;align-items:center;gap:12px;font-weight:800;font-size:17px}.mark{width:32px;height:32px;border-radius:7px;background:#2563eb;display:grid;place-items:center;font-weight:900}.topmeta{display:flex;align-items:center;gap:10px;color:#cbd5e1}
.wrap{width:100%;max-width:1180px;margin:0 auto;padding:24px}.toolbar{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:18px}.title h1{font-size:24px;margin:0 0 4px}.title p{margin:0;color:var(--muted)}.actions{display:flex;gap:10px;flex-wrap:wrap}
button{height:38px;border:0;border-radius:7px;padding:0 13px;background:var(--blue);color:white;cursor:pointer;font-weight:700}button:hover{filter:brightness(.96)}button.secondary{background:#475569}button.ghost{background:#fff;color:#1f2937;border:1px solid var(--line)}button.danger{background:var(--red)}button.small{height:30px;padding:0 10px;border-radius:6px;font-size:12px}
.status-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;margin-bottom:18px}.metric{min-width:0;background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:15px}.metric .label{color:var(--muted);font-size:12px;margin-bottom:8px}.metric .value{font-size:26px;font-weight:850;line-height:1}.metric .hint{margin-top:8px;color:var(--muted);font-size:12px}.ok{color:var(--green)}.warn{color:var(--amber)}.bad{color:var(--red)}
...
@media(max-width:980px){.form,.form.dns-fields{grid-template-columns:repeat(2,minmax(0,1fr))}.form button{grid-column:1/-1}.toolbar{align-items:flex-start;flex-direction:column}.actions{width:100%}.actions button{flex:1}.topbar{padding:0 16px}}
@media(max-width:680px){.wrap{padding:14px}.form,.form.dns-fields{grid-template-columns:1fr}.topmeta{display:none}.brand span{font-size:15px}th{display:none}tr{display:block;border-bottom:1px solid #edf0f5;padding:9px 0}td{display:flex;justify-content:space-between;gap:14px;border:0;padding:7px 14px;word-break:break-all}td:before{content:attr(data-label);color:#64748b;font-size:12px;font-weight:700;flex:0 0 auto}.namecell{font-weight:800}}
</style></head><body><div class="shell"><header class="topbar"><div class="brand"><div class="mark">N</div><span>nftables Forward Panel</span></div><div class="topmeta"><span id="last">Loading</span></div></header>
<main class="wrap"><div class="toolbar"><div class="title"><h1>Forwarding Console</h1><p>Manage nftables forwarding rules and reload state.</p></div><div class="actions"><button class="ghost" id="refreshBtn">Refresh</button><button class="secondary" id="reloadBtn">Reload nft</button></div></div>
<div class="status-grid"><div class="metric"><div class="label">nftables</div><div class="value" id="nftState">-</div><div class="hint">systemd status</div></div><div class="metric"><div class="label">Web panel</div><div class="value" id="panelState">-</div><div class="hint">panel service</div></div><div class="metric"><div class="label">Port rules</div><div class="value" id="portCount">0</div><div class="hint">DNAT entries</div></div><div class="metric"><div class="label">DNS rules</div><div class="value" id="dnsCount">0</div><div class="hint">domain entries</div></div></div>
<div id="msg" class="msg"></div><div class="stack"><section class="panel"><div class="panel-head"><h2>Add rule</h2><span class="badge green">auto reload</span></div><div class="tabs" id="formTabs"><button class="tab active" data-form="port">Port</button><button class="tab" data-form="dns">DNS</button></div><div id="portForm" class="form"><div class="field"><label>Name</label><input id="name" placeholder="web-api"></div><div class="field"><label>Local port</label><input id="lport" placeholder="10000"></div><div class="field"><label>Target port</label><input id="dport" placeholder="443"></div><div class="field"><label>Target IPv4</label><input id="dip" placeholder="1.2.3.4"></div><button id="addRuleBtn">Add port rule</button></div><div id="dnsForm" class="form dns-fields" style="display:none"><div class="field"><label>Domain</label><input id="domain" placeholder="example.com"></div><div class="field"><label>Local port</label><input id="dlport" placeholder="10001"></div><div class="field"><label>Target port</label><input id="ddport" placeholder="443"></div><div class="field"><label>nft set</label><input id="setname" placeholder="optional"></div><button id="addDnsBtn">Add DNS rule</button></div><p class="side-note">Writes nftables config and reloads the managed tables. Version <span id="panelVersion">2026.06.27.5</span></p></section>
<section class="panel"><div class="panel-head"><h2>Rules</h2><span class="badge blue" id="totalBadge">0 rules</span></div><div class="tabs" id="ruleTabs"><button class="tab active" data-tab="port">Port forwarding</button><button class="tab" data-tab="dns">DNS forwarding</button></div><div class="panel-body"><div id="portPane" class="tabpane active"><div id="rules"></div></div><div id="dnsPane" class="tabpane"><div id="dns"></div></div></div></section></div></main></div>
<script>
...
function switchTab(tab){currentTab=tab;document.querySelectorAll('#ruleTabs .tab').forEach((b,i)=>b.classList.toggle('active',(tab==='port'?i===0:i===1)));$('portPane').classList.toggle('active',tab==='port');$('dnsPane').classList.toggle('active',tab==='dns');$('totalBadge').textContent=(tab==='port'?$('portCount').textContent:$('dnsCount').textContent)+' rules'}
...
function table(rows,kind){if(!rows.length)return '<div class="empty"><strong>No rules yet</strong><span>Add a rule above to see it here.</span></div>';const isPort=kind==='rules';const heads=isPort?['Name','Entry','Target','Type','Action']:['Domain','Entry','Target port','Set','Action'];return '<table><thead><tr>'+heads.map(h=>'<th>'+h+'</th>').join('')+'</tr></thead><tbody>'+rows.map((r,i)=>{const id=isPort?r.lport:i;const target=isPort?esc(r.dip)+':'+esc(r.dport):esc(r.dport);const first=isPort?esc(r.name||'unnamed'):esc(r.domain);const set=isPort?'<span class="badge green">tcp+udp</span>':'<span class="badge amber">'+esc(r.set_name)+'</span>';return '<tr><td data-label="'+heads[0]+'" class="namecell">'+first+'</td><td data-label="'+heads[1]+'"><span class="badge blue">'+esc(r.lport)+'</span></td><td data-label="'+heads[2]+'" class="target">'+target+'</td><td data-label="'+heads[3]+'">'+set+'</td><td data-label="Action"><button class="danger small delbtn" data-kind="'+kind+'" data-id="'+esc(id)+'">Delete</button></td></tr>'}).join('')+'</tbody></table>'}
async function load(){try{const d=await api('/api/state');$('nftState').textContent=d.nftables||'-';$('panelState').textContent=d.panel||'-';$('nftState').className='value '+stateClass(d.nftables);$('panelState').className='value '+stateClass(d.panel);$('portCount').textContent=d.port_rules.length;$('dnsCount').textContent=d.dns_rules.length;$('rules').innerHTML=table(d.port_rules,'rules');$('dns').innerHTML=table(d.dns_rules,'dns');$('last').textContent='Last refresh '+new Date().toLocaleTimeString();switchTab(currentTab)}catch(e){$('last').textContent='Load failed';showMsg(e.message,'error')}}
async function addRule(){try{await api('/api/rules',{method:'POST',body:JSON.stringify({name:$('name').value,lport:$('lport').value,dip:$('dip').value,dport:$('dport').value})});['name','lport','dip','dport'].forEach(id=>$(id).value='');showMsg('Port rule added and reloaded.');load()}catch(e){showMsg(e.message,'error')}}
async function addDns(){try{await api('/api/dns',{method:'POST',body:JSON.stringify({domain:$('domain').value,lport:$('dlport').value,dport:$('ddport').value,set_name:$('setname').value})});['domain','dlport','ddport','setname'].forEach(id=>$(id).value='');showMsg('DNS rule added and reloaded.');load()}catch(e){showMsg(e.message,'error')}}
async function del(kind,id){if(!confirm('Delete this rule?'))return;try{await api('/api/'+kind+'/'+encodeURIComponent(id),{method:'DELETE'});showMsg('Rule deleted and reloaded.');load()}catch(e){showMsg(e.message,'error')}}
async function reloadRules(){try{await api('/api/reload',{method:'POST'});showMsg('nftables reloaded.');load()}catch(e){showMsg(e.message,'error')}}
